...
<html>
	...
	<body>
		...
		<form action="./toUploadImage.php" method="post" target="upload_image_iframe" id="image_upload_form" enctype="multipart/form-data">
		...
		<!--
			...
			iUploadIndex=<?php echo isset($_GET['upload_index'])?$_GET['upload_index']:0; ?>,
			oUploadLoaddingWrap=$('#upload_loadding_wrap'), // 显示现在上传
			oHasLoadedWrap=$('#image_has_loaded_wrap'),
			oParentWin=window.top, // 父窗口
			oUploadFileField=$('li.image-upload-filed-wrap:eq('+iUploadIndex+')',oParentWin.document), // 存放图片列表
			oUploadedCount=document.getElementById('hidden_uploaded_count'), // 已经上传的图片数量
			oFileField=$('#image_upload_file_field').change(function (){
				
				oUploadedCount.value=$('div.upload-image-list-wrap:eq('+iUploadIndex+')',oParentWin.document).find('li.signle-image-public-wrap-no-style-for-js').size();
			...
			function afterUpload(aImgList){
				...
				// 调用父窗口的上传成功后的方法
				typeof oParentWin.afterUpload=='function' && oParentWin.afterUpload(aImgList,iUploadIndex,'<?php echo isset($_GET['input_hidden_name'])?htmlspecialchars($_GET['input_hidden_name']):'img_list'; ?>',<?php echo I_MAX_UPLOAD_COUNT; ?>);
